...
def tee_object_pos(env: ManagerBasedEnv, _: Any) -> torch.Tensor:
    ...
    tee: RigidObject = env.scene[SceneEntityCfg("tee_object").name]
    tee_pos_w = tee.data.root_pos_w
    return tee_pos_w
...
def tee_object_rot(env: ManagerBasedEnv, _: Any) -> torch.Tensor:
    ...
    tee: RigidObject = env.scene[SceneEntityCfg("tee_object").name]
    tee_rot_w = tee.data.root_quat_w
    return tee_rot_w


def tee_marker_pos(env: ManagerBasedEnv, _: Any) -> torch.Tensor:
    """Get the position of the T-shaped marker in world frame."""
    tee_marker: RigidObject = env.scene[SceneEntityCfg("tee_marker").name]
    tee_marker_pos_w = tee_marker.data.root_pos_w
    return tee_marker_pos_w


def tee_marker_rot(env: ManagerBasedEnv, _: Any) -> torch.Tensor:
    """Get the orientation of the T-shaped marker in world frame."""
    tee_marker: RigidObject = env.scene[SceneEntityCfg("tee_marker").name]
    tee_marker_rot_w = tee_marker.data.root_quat_w
    return tee_marker_rot_w
...
def goal_quat_diff(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, command_name: str, make_quat_unique: bool) -> torch.Tensor:
    ...
    tee_quat_w = tee.data.root_quat_w

    # compute quaternion difference
    quat = math_utils.quat_mul(tee_quat_w, math_utils.quat_conjugate(goal_quat_w))
