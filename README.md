...
- Throughout the repository, the name `pushT_isaacsim` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of pushT_isaacsim (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/pushT_isaacsim
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/pushT_isaacsim/pushT_isaacsim/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/pushT_isaacsim"
    ]
}
```
